...
local initcheck = argcheck{
   ...
   {check=function(pathsBlur)
       local out = true;
       for k,v in ipairs(pathsBlur) do
          if type(v) ~= 'string' then
             print('pathsBlur can only be of string input');
             out = false
          end
       end
       return out
   ...
   {name="Trajectory_labels",
    type="torch.FloatTensor",
    help="Trajectory labels",
    opt = true},    
...
function dataset:__init(...)
   ...
   if not self.loadSize then self.loadSize = 224; end
...
function dataset:getByClass(class)
 local index = math.ceil(torch.uniform() * self.classListSample[class]:nElement())
   ...
   --local imgpath='/media/dataset/Nimisha/Motion_estimation_dataset/facedataset_trans/Train/train/' .. index..'.png'
   --local imgpath1 = ffi.string(torch.data(self.imagePath[self.classListSample[class][index]])) --nim
   --local imgpath1='/media/dataset/Nimisha/Motion_estimation_dataset/facedataset_trans/Train/trainb/' .. index..'.png'
   
   local imgpath=self.paths[1] .. index .. '.png'
   local imgpath1=self.pathsBlur[1] .. index .. '.png'
   local iname = imgpath:split("/")
   local imgname = iname[#iname]
   imgname = string.sub(imgname, 1, #imgname-4)
   n_img = tonumber(imgname)
   local label_vec = self.Trajectory_labels[{{n_img}}]
   label_vec = torch.reshape(label_vec, 1, label_vec:size()[2])
   return self:sampleHookTrain(imgpath),self:sampleHookTrain(imgpath1), label_vec
end

-- converts a table of samples (and corresponding labels) to a clean tensor
local function tableToOutput(self, dataTable, dataTableb, scalarTable, trajTable) --nim
   local data,datab, scalarLabels, trajLabels, labels
   local quantity = #scalarTable
   local traj_len = (trajTable[1]):size()[2]
   
   assert(dataTable[1]:dim() == 3)
   assert(dataTableb[1]:dim() == 3)
   data = torch.Tensor(quantity,
		       self.loadSize[1], self.loadSize[2], self.loadSize[3])
   datab = torch.Tensor(quantity,
		       self.loadBlurSize[1], self.loadBlurSize[2], self.loadBlurSize[3]) --nim
   scalarLabels = torch.LongTensor(quantity):fill(-1111)
   trajLabels = torch.Tensor(quantity, traj_len)
   
   for i=1,#dataTable do
      data[i]:copy(dataTable[i])
      datab[i]:copy(dataTableb[i])
      scalarLabels[i] = scalarTable[i]
      trajLabels[i] = trajTable[i]
   end
   return data, datab, scalarLabels, trajLabels
end
...
function dataset:sample(quantity)
   ...
   for i=1,quantity do
      ...
      local out,outb, tlab = self:getByClass(class)
      table.insert(dataTable, out)
      table.insert(dataTableb, outb)
      table.insert(trajTable, tlab)
      table.insert(scalarTable, class)
   end
   local data,datab, scalarLabels, trajLabels = tableToOutput(self, dataTable,dataTableb, scalarTable, trajTable)
   return data, datab, scalarLabels, trajLabels
end
...
--function dataset:get(i1, i2)
 ...
 --  for i=1,quantity do
  --    -- load the sample
 --   local imgpath =self.paths[1] .. indices[i] .. '.png'
 --   local imgpath1=self.pathsBlur[1] .. indices[i] .. '.png'        --ffi.string(torch.data(self.imagePath[indices[i]]))
 --   local iname = imgpath:split("/")
--    local imgname = iname[#iname]
--    imgname = string.sub(imgname, 1, #imgname-4)
--    local label_vec = self.Trajectory_labels[tonumber(imgname)]
--    local out, tlab = self:sampleHookTest(imgpath)
--    local outb, tlab = self:sampleHookTest(imgpath1)
--    table.insert(dataTable, out)
--    table.insert(dataTableb, outb)
--    table.insert(trajTable, tlab)
